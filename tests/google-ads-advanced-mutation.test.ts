...
test("RSA limits and target gate are enforced before API mutation", () => {
  ...
  const rsa = buildResponsiveSearchAd({ adGroupId: "2", adId: "1", headlines: ["Título A", "Título B", "Título C"], descriptions: ["Descrição A", "Descrição B"], finalUrls: ["https://example.com"], finalMobileUrls: [] }, { resourceName: "customers/1234567890/ads/1", adType: "RESPONSIVE_SEARCH_AD", headlineAssets: [{ text: "Título A", pinnedField: "HEADLINE_1" }], headlines: ["Anterior A", "Anterior B", "Anterior C"], descriptions: ["Descrição A", "Descrição B"], finalUrls: ["https://example.com"], finalMobileUrls: [] });
  assert.equal(rsa.collection, "ads");
  assert.equal(rsa.operations[0].updateMask, "responsive_search_ad.headlines,responsive_search_ad.descriptions,final_urls");
  assert.deepEqual(rsa.operations[0].update, { resourceName: "customers/1234567890/ads/1", responsiveSearchAd: { headlines: [{ text: "Título A", pinnedField: "HEADLINE_1" }, { text: "Título B" }, { text: "Título C" }], descriptions: [{ text: "Descrição A" }, { text: "Descrição B" }] }, finalUrls: ["https://example.com"] });
  assert.deepEqual(rsa.target, { adGroupId: "2", adId: "1", resourceName: "customers/1234567890/ads/1", headlines: ["Título A", "Título B", "Título C"], descriptions: ["Descrição A", "Descrição B"], finalUrls: ["https://example.com"], finalMobileUrls: [] });
  assert.throws(() => buildResponsiveSearchAd({ adGroupId: "2", adId: "1", headlines: ["Título A", "Título B", "Título C"], descriptions: ["Descrição A", "Descrição B"], finalUrls: ["https://example.com"], finalMobileUrls: ["https://m.example.com"] }, { resourceName: "customers/1234567890/ads/1", adType: "RESPONSIVE_SEARCH_AD", headlines: ["Anterior A", "Anterior B", "Anterior C"], descriptions: ["Descrição A", "Descrição B"], finalUrls: ["https://example.com"], finalMobileUrls: [] }), /URLs mobile/);
