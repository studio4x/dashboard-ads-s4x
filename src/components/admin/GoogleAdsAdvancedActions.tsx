...
type Values = {
  ...
  rsaApplicationMode: "replace" | "create_pause" | "create_keep";
  dayOfWeek: string;
  start: string;
  end: string;
  geoSearch: string;
  ...
  return <section ref={rootRef} id="google-ads-optimization-center" data-s4x-google-ads-advanced-actions="true" data-s4x-action-index={actionIndex} className="card" style={{ width: "100%", boxSizing: "border-box", margin: "18px 0", padding: 20, border: "1px solid #BFDBFE", borderRadius: 12, background: "linear-gradient(180deg,#FFFFFF 0%,#F8FBFF 100%)", overflow: "hidden", scrollMarginTop: 24 }}>
    ...
    {assistantOpen ? <section style={{ marginTop: 14, borderTop: "1px solid #E2E8F0", paddingTop: 12 }}>
      ...
      <div style={{ marginTop: 11, display: "grid", gap: 10 }}>
        ...
        {assistantSection === "ads" ? <><Grid><Field label="Anúncio responsivo"><select value={values.adId} onChange={(event) => selectAd(event.target.value)} style={fieldStyle()}>{context.ads.filter((item) => item.type === "RESPONSIVE_SEARCH_AD").map((item) => <option key={`${item.adGroupId}:${item.id}`} value={item.id}>{item.campaignName} · {item.name}</option>)}</select></Field><Field label="Grupo"><input value={selectedAd?.adGroupName || ""} readOnly style={{ ...fieldStyle(), background: "#F8FAFC" }} /></Field></Grid><Field label="Como aplicar"><select value={values.rsaApplicationMode} onChange={(event) => setValue("rsaApplicationMode", event.target.value as Values["rsaApplicationMode"])} style={fieldStyle()}><option value="replace">Substituir conteúdo do anúncio existente</option><option value="create_pause">Criar novo anúncio e pausar o existente</option><option value="create_keep">Criar novo anúncio e manter o existente ativo</option></select></Field><p style={{ margin: 0, padding: "7px 9px", borderRadius: 7, background: values.rsaApplicationMode === "replace" ? "#EFF6FF" : values.rsaApplicationMode === "create_pause" ? "#FFF7ED" : "#F0FDF4", color: "#475569", fontSize: 9.5, lineHeight: 1.45 }}>{values.rsaApplicationMode === "replace" ? "Mantém o mesmo anúncio e troca seus títulos, descrições e URLs." : values.rsaApplicationMode === "create_pause" ? "Cria um anúncio com novo ID e pausa o atual somente se a criação for aceita." : "Cria um anúncio com novo ID e mantém os dois ativos para veiculação."}</p><Grid><Field label="Títulos (um por linha)"><textarea value={values.headlines} onChange={(event) => setValue("headlines", event.target.value)} style={{ ...fieldStyle(), height: 110, padding: 9, resize: "vertical" }} /></Field><Field label="Descrições (uma por linha)"><textarea value={values.descriptions} onChange={(event) => setValue("descriptions", event.target.value)} style={{ ...fieldStyle(), height: 110, padding: 9, resize: "vertical" }} /></Field></Grid><Grid><Field label="URLs finais"><textarea value={values.finalUrls} onChange={(event) => setValue("finalUrls", event.target.value)} style={{ ...fieldStyle(), height: 68, padding: 9 }} /></Field><Field label="URLs mobile"><textarea value={values.finalMobileUrls} onChange={(event) => setValue("finalMobileUrls", event.target.value)} style={{ ...fieldStyle(), height: 68, padding: 9 }} /></Field></Grid><div style={{ display: "flex", gap: 7, flexWrap: "wrap" }}><button type="button" disabled={isLoading || !selectedAd} onClick={() => selectedAd && void draftRsa(selectedAd)} style={buttonStyle("neutral")}><Sparkles size={12} style={{ verticalAlign: "middle", marginRight: 5 }} />Gerar rascunho com IA</button><button type="button" disabled={isLoading || !selectedAd} onClick={() => void prepare(values.rsaApplicationMode === "replace" ? "edit_responsive_search_ad" : "create_responsive_search_ad", { adGroupId: values.adGroupId, adId: values.adId, pauseExisting: values.rsaApplicationMode === "create_pause", headlines: asLines(values.headlines), descriptions: asLines(values.descriptions), finalUrls: asLines(values.finalUrls), finalMobileUrls: asLines(values.finalMobileUrls) })} style={buttonStyle()}>{values.rsaApplicationMode === "replace" ? "Preparar substituição do anúncio" : "Preparar criação do novo anúncio"}</button></div></> : null}
