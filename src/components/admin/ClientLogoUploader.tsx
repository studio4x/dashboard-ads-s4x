  ...
  return (
    <div className="card" style={{ padding: 24 }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        <div onClick={() => inputRef.current?.click()} style={{ width: "100%", height: 170, borderRadius: 12, border: "2px dashed #CBD5E1", background: "#F8FAFC", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", overflow: "hidden" }}>{preview ? <div style={{ display: "grid", gap: 8, justifyItems: "center" }}><span style={{ color: "#64748B", fontSize: 11, fontWeight: 650 }}>Prévia no PDF</span><div style={{ width: 180, height: 44, borderRadius: 8, background: "#FFFFFF", boxShadow: "0 6px 18px rgba(15,23,42,.14)", overflow: "hidden" }}><img src={preview} alt={`Logo ${clientName}`} style={{ display: "block", width: "100%", height: "44px", ...getClientLogoImageStyle(settings) }} /></div></div> : <div style={{ textAlign: "center", color: "#94A3B8" }}><Upload size={30} style={{ margin: "0 auto 8px" }} /><p style={{ fontSize: 13 }}>Clique para selecionar</p></div>}</div>
