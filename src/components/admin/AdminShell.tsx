...
export function AdminShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#F8FAFC", display: "flex" }}>
        ...
        <>
          ...
          >
            <button
              ...
              style={{ border: "1px solid #E2E8F0", background: "#FFFFFF", borderRadius: 8, width: 36, height: 36, display: "flex", alignItems: "center", justifyContent: "center", color: "#334155" }}
            >
              <Menu size={18} />
            </button>
            <p style={{ fontSize: 14, fontWeight: 700, color: "#0F172A" }}>{pageTitle}</p>
          ...
          {sidebarOpen && (
            <div style={{ position: "fixed", inset: 0, zIndex: 80, display: "flex" }}>
              <button
                ...
                style={{ position: "absolute", inset: 0, border: "none", background: "rgba(15,23,42,0.45)" }}
              />
      ...
      <main style={{ flex: 1, minWidth: 0, overflowX: "hidden", paddingTop: isMobile ? 56 : 0 }}>
        {children}
      </main>
    </div>
  );
