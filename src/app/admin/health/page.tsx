...
function sourceHref(source: any) {
  if (source.type === "google_ads") return `/admin/google-ads-api/analysis/${encodeURIComponent(source.id)}`;
  return `/admin/clients/${encodeURIComponent(source.clientId)}/visao-geral#source-${encodeURIComponent(source.id)}`;
}
  ...
  return (
    <div className="admin-page" style={{ padding: "clamp(14px, 3vw, 32px)", maxWidth: 1380 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(420px,1fr))", gap: 14 }}>
        <div className="card" style={{ overflow: "auto" }}><div style={{ padding: 13, borderBottom: "1px solid #E2E8F0", fontWeight: 800, fontSize: 13, color: "#0F172A" }}>Fontes de dados</div><table style={{ width: "100%", borderCollapse: "collapse", fontSize: 10, minWidth: 620 }}><thead><tr style={{ background: "#F8FAFC", textAlign: "left" }}><th style={{ padding: 9 }}>Cliente / Fonte</th><th style={{ padding: 9 }}>Última atualização</th><th style={{ padding: 9 }}>Status</th><th style={{ padding: 9, textAlign: "right" }}>Ação</th></tr></thead><tbody>{(snapshot?.sources || []).map((source: any) => <tr key={source.id} style={{ borderTop: "1px solid #F1F5F9" }}><td style={{ padding: 9 }}><strong>{source.clientName}</strong><div style={{ color: "#64748B" }}>{source.name}</div></td><td style={{ padding: 9 }}>{formatDate(source.freshAt)}{source.ageMinutes !== null && <div style={{ color: "#94A3B8" }}>{source.ageMinutes} min</div>}</td><td style={{ padding: 9 }}><Badge status={source.status} /></td><td style={{ padding: 9, textAlign: "right" }}><Link href={sourceHref(source)} target="_blank" rel="noreferrer" title={`Abrir fonte ${source.name}`} aria-label={`Abrir fonte ${source.name}`} style={{ display: "inline-flex", alignItems: "center", gap: 5, padding: "6px 8px", borderRadius: 7, background: "#EFF6FF", color: "#1D4ED8", fontSize: 10, fontWeight: 700, textDecoration: "none", whiteSpace: "nowrap" }}><ExternalLink size={12} /> Abrir fonte</Link></td></tr>)}</tbody></table></div>
