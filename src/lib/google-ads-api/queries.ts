...
export const googleAdsAnalyticsQueries = {
  campaignDaily: (start: string, end: string) => `SELECT segments.date, campaign.id, campaign.name, campaign.status, campaign.primary_status, campaign.primary_status_reasons, campaign.advertising_channel_type, campaign.advertising_channel_sub_type, campaign.bidding_strategy_type, campaign.bidding_strategy, ${ANALYTIC_METRICS}, ${SEARCH_SHARE_METRICS} FROM campaign WHERE ${between(start, end)} ORDER BY segments.date, campaign.id`,
  ...
  locationDaily: (start: string, end: string) => `SELECT segments.date, segments.geo_target_country, segments.geo_target_region, segments.geo_target_city, segments.geo_target_metro, segments.geo_target_most_specific_location, geographic_view.country_criterion_id, geographic_view.location_type, campaign.id, campaign.name, ${ANALYTIC_METRICS} FROM geographic_view WHERE ${between(start, end)} ORDER BY segments.date, campaign.id`,
  ...
  conversionActionDaily: (start: string, end: string) => `SELECT segments.date, segments.conversion_action, segments.conversion_action_name, segments.conversion_action_category, segments.external_conversion_source, campaign.id, campaign.name, metrics.conversions, metrics.all_conversions, metrics.conversions_value, metrics.all_conversions_value, metrics.cost_per_conversion, metrics.value_per_conversion FROM campaign WHERE ${between(start, end)} ORDER BY segments.date, campaign.id, segments.conversion_action`,
  conversionActions: `SELECT conversion_action.resource_name, conversion_action.id, conversion_action.name, conversion_action.status, conversion_action.type, conversion_action.category, conversion_action.origin, conversion_action.primary_for_goal, conversion_action.include_in_conversions_metric, conversion_action.counting_type, conversion_action.attribution_model_settings FROM conversion_action ORDER BY conversion_action.id`,
