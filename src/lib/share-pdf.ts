...
function traceChromiumRuntimeAssets() {
  if (process.env.CHROME_EXECUTABLE_PATH) return;
  try {
    // Keep the Brotli assets attached only to the renderer route in NFT output.
    statSync("node_modules/@sparticuz/chromium/bin/al2023.tar.br");
    statSync("node_modules/@sparticuz/chromium/bin/chromium.br");
    statSync("node_modules/@sparticuz/chromium/bin/fonts.tar.br");
    statSync("node_modules/@sparticuz/chromium/bin/swiftshader.tar.br");
  } catch {
    // Runtime resolution is handled by @sparticuz/chromium from its package path.
  }
}
...
function renderFinancialStatuses(statuses: Array<Record<string, unknown>> | undefined) {
  ...
  const cards = statuses.map((status) => {
    ...
    const estimatedEndDate = status.estimatedEndDate || (isGoogle ? calculateEstimatedEndDate({
      ...
      asOfDate: status.updatedAt ? String(status.updatedAt).slice(0, 10) : null,
    }) : null);
    ...
    const details = [
      ...
      pdfDate(estimatedEndDate) ? `Data estimada de término: ${pdfDate(estimatedEndDate)}` : null,
    ].filter(Boolean).join(" · ");
