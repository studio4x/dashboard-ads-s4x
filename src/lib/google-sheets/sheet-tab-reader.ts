  ...
  readPerformanceDailyMetaS4X(rows: any[][]): NormalizedSheetData<MetaAdsS4XDailyPerformance> {
    ...
    const data = rows.slice(1)
      ...
      .map((row) => mapRowToModel(headers, row, (raw) => ({
        ...
        messagingConversationsStarted: SheetNormalizer.toInteger(raw["Messaging Conversations Started"]),
        costPerMessagingConversationsStarted: SheetNormalizer.toNumber(raw["Cost per Messaging Conversations Started"]),
        leads: SheetNormalizer.toInteger(raw["Leads"]),
        onFacebookLeads: SheetNormalizer.toInteger(raw["On-Facebook Leads"]),
        websiteLeads: SheetNormalizer.toInteger(raw["Website Leads"]),
        offlineLeads: SheetNormalizer.toInteger(raw["Offline Leads"]),
        costPerLead: SheetNormalizer.toNumber(raw["Cost per Lead"]),
        costPerOnFacebookLead: SheetNormalizer.toNumber(raw["Cost per On-Facebook Lead"]),
        costPerWebsiteLead: SheetNormalizer.toNumber(raw["Cost per Website Lead"]),
        costPerOfflineLead: SheetNormalizer.toNumber(raw["Cost per Offline Lead"]),
