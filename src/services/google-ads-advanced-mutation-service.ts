...
async function stateFor(ctx: Context, operation: GoogleAdsAdvancedOperation, target: Record<string, unknown>): Promise<Record<string, unknown>> {
  ...
  if (operation === "edit_responsive_search_ad" || operation === "create_responsive_search_ad") {
    const adId = digits(target.adId); const adGroupId = digits(target.adGroupId); const resourceName = text(target.resourceName);
    const predicate = adId && adGroupId
      ? `ad_group_ad.ad.id = ${adId} AND ad_group.id = ${adGroupId}`
      : /^customers\/\d+\/ads\/\d+$/.test(resourceName)
        ? `ad_group_ad.ad.resource_name = '${escape(resourceName)}'`
        : null;
    if (!predicate) throw new Error("O rascunho não contém o identificador do anúncio necessário para confirmar a alteração.");
    const row = await first(ctx, `SELECT ad_group.resource_name, ad_group_ad.resource_name, ad_group_ad.status, ad_group_ad.ad.resource_name, ad_group_ad.ad.type, ad_group_ad.ad.final_urls, ad_group_ad.ad.final_mobile_urls, ad_group_ad.ad.responsive_search_ad.headlines, ad_group_ad.ad.responsive_search_ad.descriptions FROM ad_group_ad WHERE ${predicate} LIMIT 1`); const adGroup = object(row?.adGroup); const adGroupAd = object(row?.adGroupAd); const ad = object(adGroupAd.ad); const rsa = object(ad.responsiveSearchAd);
