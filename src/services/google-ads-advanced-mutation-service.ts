...
import { buildAdGroupKeyword, buildAdSchedule, buildCampaignAsset, buildConversionGoal, buildKeywordCpc, buildLocation, buildResponsiveSearchAd, buildTargetOrBidding, type AdvancedBuild } from "@/lib/google-ads-api/advanced-mutation-builders";
import { googleAdsHighRiskWritesEnabled, googleAdsRiskForOperation, googleAdsWritesEnabled, requiresStrongGoogleAdsConfirmation } from "@/lib/google-ads-api/risk-policy";
...
async function stateFor(ctx: Context, operation: GoogleAdsAdvancedOperation, target: Record<string, unknown>): Promise<Record<string, unknown>> {
  ...
  if (operation === "add_ad_group_keyword") {
    ...
    return { customerId: ctx.customerId, campaignResourceName: text(object(groupRow?.campaign).resourceName), campaignName: text(object(groupRow?.campaign).name), adGroupResourceName: text(object(groupRow?.adGroup).resourceName), adGroupName: text(object(groupRow?.adGroup).name), duplicate: criteria.some((r) => text(object(r.adGroupCriterion?.keyword).text).toLocaleLowerCase("pt-BR") === keyword && text(object(r.adGroupCriterion?.keyword).matchType).toUpperCase() === mt), conflict: negatives.some((r) => text(object(r.campaignCriterion?.keyword).text).toLocaleLowerCase("pt-BR") === keyword && text(object(r.campaignCriterion?.keyword).matchType).toUpperCase() === mt) };
  }
  ...
  if (["add_ad_schedule", "edit_ad_schedule", "remove_ad_schedule"].includes(operation)) {
    ...
    return { campaignResourceName: text(object(campaignRow?.campaign).resourceName), existingSchedules: schedules.map((r) => { const s = object(r.campaignCriterion?.adSchedule); return { resourceName: r.campaignCriterion?.resourceName, dayOfWeek: s.dayOfWeek, startMinutes: Number(s.startHour || 0) * 60 + Number(String(s.startMinute || "ZERO").replace(/\D/g, "") || 0), endMinutes: Number(s.endHour || 0) * 60 + Number(String(s.endMinute || "ZERO").replace(/\D/g, "") || 0) }; }) };
  }
  if (["add_campaign_location", "remove_campaign_location"].includes(operation)) {
    ...
    return { campaignResourceName: text(object(campaignRow?.campaign).resourceName), existingLocations: locations.map((r) => ({ resourceName: r.campaignCriterion?.resourceName, geoTargetConstantResourceName: r.campaignCriterion?.location?.geoTargetConstant })) };
  }
  if (operation === "edit_responsive_search_ad") {
    const row = await first(ctx, `SELECT ad_group_ad.resource_name, ad_group_ad.ad.type, ad_group_ad.ad.final_urls, ad_group_ad.ad.final_mobile_urls, ad_group_ad.ad.responsive_search_ad.headlines, ad_group_ad.ad.responsive_search_ad.descriptions FROM ad_group_ad WHERE ad_group_ad.ad.id = ${digits(target.adId)} AND ad_group.id = ${digits(target.adGroupId)} LIMIT 1`); const ad = object(row?.adGroupAd?.ad); const rsa = object(ad.responsiveSearchAd);
    return { resourceName: row?.adGroupAd?.resourceName, adType: ad.type, finalUrls: (ad.finalUrls || []).map((v: unknown) => text(v)), finalMobileUrls: (ad.finalMobileUrls || []).map((v: unknown) => text(v)), headlines: (rsa.headlines || []).map((v: Row) => text(v.text)), descriptions: (rsa.descriptions || []).map((v: Row) => text(v.text)) };
  }
  ...
  if (["set_target_cpa", "set_target_roas", "set_campaign_bidding_strategy"].includes(operation)) {
    ...
    return { resourceName: campaign.resourceName, biddingStrategyType: campaign.biddingStrategyType, targetCpaMicros: campaign.maximizeConversions?.targetCpaMicros || campaign.targetCpa?.targetCpaMicros, targetRoas: campaign.maximizeConversionValue?.targetRoas || campaign.targetRoas?.targetRoas, recentConversions: metrics.reduce((sum, r) => sum + Number(r.metrics?.conversions || 0), 0), campaignType: campaign.advertisingChannelType };
  }
...
function requiredConfirmation(risk: GoogleAdsRiskLevel) { return requiresStrongGoogleAdsConfirmation(risk) ? risk === "critical" ? "CONFIRMAR ALTERAÇÃO CRÍTICA" : "CONFIRMAR ALTERAÇÃO DE LANCES" : null; }
...
async function readBack(ctx: Context, operation: GoogleAdsAdvancedOperation, target: Record<string, unknown>, change: AdvancedBuild, body: unknown) {
  ...
  else if (resourceName && operation === "edit_responsive_search_ad") query = `SELECT ad_group_ad.resource_name FROM ad_group_ad WHERE ad_group_ad.resource_name = '${escape(resourceName)}' LIMIT 1`;
...
export const GoogleAdsAdvancedMutationService = {
  async recommendations(sourceId: string) {
    ...
    return result.slice(0, 100).map((row) => ({ resourceName: text(row.recommendation?.resourceName), type: text(row.recommendation?.type), dismissed: Boolean(row.recommendation?.dismissed), impact: row.recommendation?.impact || null, details: row.recommendation || null, origin: "GOOGLE_RECOMMENDATION" }));
  },
  ...
  async execute(input: { requestId: string; previewHash: string; confirmation: string; actorId: string | null; actorRole?: string | null }) {
    ...
    const operation = text(request.operation_type) as GoogleAdsAdvancedOperation; if (!isAdvanced(operation)) throw new Error("Operação avançada inválida."); const risk = googleAdsRiskForOperation(operation, object(request.target)); const required = requiredConfirmation(risk); if (required && input.confirmation.trim().toUpperCase() !== required) throw new Error(`Digite exatamente “${required}” para confirmar esta alteração.`); if (risk === "critical" && input.actorRole !== "owner") throw new Error("Somente owner pode executar alterações críticas de conversão.");
