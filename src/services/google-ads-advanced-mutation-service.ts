...
async function stateFor(ctx: Context, operation: GoogleAdsAdvancedOperation, target: Record<string, unknown>): Promise<Record<string, unknown>> {
  ...
  if (operation === "edit_responsive_search_ad") {
    const row = await first(ctx, `SELECT ad_group_ad.resource_name, ad_group_ad.ad.type, ad_group_ad.ad.final_urls, ad_group_ad.ad.final_mobile_urls, ad_group_ad.ad.responsive_search_ad.headlines, ad_group_ad.ad.responsive_search_ad.descriptions FROM ad_group_ad WHERE ad_group_ad.ad.id = ${digits(target.adId)} AND ad_group.id = ${digits(target.adGroupId)} LIMIT 1`); const adGroupAd = object(row?.adGroupAd); const ad = object(adGroupAd.ad); const rsa = object(ad.responsiveSearchAd);
    return { resourceName: adGroupAd.resourceName, adType: ad.type, finalUrls: (Array.isArray(ad.finalUrls) ? ad.finalUrls : []).map(text), finalMobileUrls: (Array.isArray(ad.finalMobileUrls) ? ad.finalMobileUrls : []).map(text), headlines: (Array.isArray(rsa.headlines) ? rsa.headlines : []).map((v) => text(object(v).text)), descriptions: (Array.isArray(rsa.descriptions) ? rsa.descriptions : []).map((v) => text(object(v).text)) };
  }
